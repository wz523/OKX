...
TREND_DAILY_CAP = 5   # 0 表示不限制每日次数
TREND_COOLDOWN_SEC = 180  # 信号之间无冷却
...
# 新增：最大名义敞口硬上限（可热更新）
# ------------------------------------------------------------------
MAX_NOTIONAL_USD_LONG  = Decimal("500")
MAX_NOTIONAL_USD_SHORT = Decimal("500")


# ========== 应用 JSONC（被策略循环调用） ==========

def apply_tuning(cfg_dict: Dict[str, Any], mkt, acc, grid, guard):
    ...
    global MAX_NOTIONAL_USD_LONG, MAX_NOTIONAL_USD_SHORT

    # 名义/乘数
    USE_TARGET_NOTIONAL = bool(cfg_dict.get("use_target_notional", USE_TARGET_NOTIONAL))
    TARGET_NOTIONAL_USD_LONG = to_decimal(cfg_dict.get("target_notional_usd_long", TARGET_NOTIONAL_USD_LONG))
    TARGET_NOTIONAL_USD_SHORT = to_decimal(cfg_dict.get("target_notional_usd_short", TARGET_NOTIONAL_USD_SHORT))
    BASE_MULTIPLIER_LONG = int(cfg_dict.get("base_multiplier_long", BASE_MULTIPLIER_LONG))
    BASE_MULTIPLIER_SHORT = int(cfg_dict.get("base_multiplier_short", BASE_MULTIPLIER_SHORT))

    # 趋势加仓参数
    TREND_NOTIONAL_USD = to_decimal(cfg_dict.get("trend_notional_usd", TREND_NOTIONAL_USD))
    TREND_DAILY_CAP = int(cfg_dict.get("trend_daily_cap", TREND_DAILY_CAP))
    TREND_COOLDOWN_SEC = int(cfg_dict.get("trend_cooldown_sec", TREND_COOLDOWN_SEC))
    TREND_MAX_NOTIONAL_USD = to_decimal(cfg_dict.get("trend_max_notional_usd", TREND_MAX_NOTIONAL_USD))
    TREND_MIN_DISTANCE_PCT = to_decimal(cfg_dict.get("trend_min_distance_pct", TREND_MIN_DISTANCE_PCT))
    # 保留旧字段赋值，避免兼容问题
    TREND_USE_TARGET = True
    TREND_TARGET_USD_LONG = TREND_NOTIONAL_USD
    TREND_TARGET_USD_SHORT = TREND_NOTIONAL_USD
    ...
    # 风险档位
    rp = str(cfg_dict.get("risk_profile", RISK_PROFILE)).lower().strip()
    ...
    RISK_SCALE_GRID = to_decimal(cfg_dict.get("risk_scale_grid", grid_scale_default))
    RISK_SCALE_TREND = to_decimal(cfg_dict.get("risk_scale_trend", trend_scale_default))

    # 网格形态
    old_step = GRID_STEP_USD; old_levels = GRID_LEVELS_PER_SIDE
    GRID_STEP_USD = to_decimal(cfg_dict.get("grid_step_usd", GRID_STEP_USD))
    GRID_LEVELS_PER_SIDE = int(cfg_dict.get("grid_levels_per_side", GRID_LEVELS_PER_SIDE))
    RECENTER_PCT = to_decimal(cfg_dict.get("recenter_pct", RECENTER_PCT))
    POST_ONLY = bool(cfg_dict.get("post_only", POST_ONLY))

    # 打印/日志
    SIGNAL_PRINT_MODE = cfg_dict.get("signal_print_mode", SIGNAL_PRINT_MODE)
    SIGNAL_HEARTBEAT_EVERY = int(cfg_dict.get("signal_heartbeat_every", SIGNAL_HEARTBEAT_EVERY))
    POSITION_LEDGER_ENABLE = bool(cfg_dict.get("position_ledger_enable", POSITION_LEDGER_ENABLE))
    CONFIRM_FILL_TIMEOUT_MS = int(cfg_dict.get("confirm_fill_timeout_ms", CONFIRM_FILL_TIMEOUT_MS))

    # 盈利/止盈
    TP_BASE_USD = to_decimal(cfg_dict.get("tp_base_usd", TP_BASE_USD))
    TP_PARTIAL_RATIO = to_decimal(cfg_dict.get("tp_partial_ratio", TP_PARTIAL_RATIO))
    TP_TRAIL_PCT = to_decimal(cfg_dict.get("tp_trail_pct", TP_TRAIL_PCT))
    TP_TRAIL_USD = to_decimal(cfg_dict.get("tp_trail_usd", TP_TRAIL_USD))
    # 保留旧字段赋值，避免报错
    SCALP_TRIGGER_USD = TP_BASE_USD
    PARTIAL_TP_RATIO = TP_PARTIAL_RATIO
    TRAIL_CALLBACK_PCT = TP_TRAIL_PCT
    TRAIL_CALLBACK_USD_FLOOR = TP_TRAIL_USD
    USE_CLOSE_POSITION_FOR_FLAT = bool(cfg_dict.get("use_close_position_for_flat", USE_CLOSE_POSITION_FOR_FLAT))
    ALLOW_TAKER_ON_BAND = bool(cfg_dict.get("allow_taker_on_band", ALLOW_TAKER_ON_BAND))

    # DCA
    DCA_ENABLE = bool(cfg_dict.get("dca_enable", DCA_ENABLE))
    DCA_MIN_PCT = to_decimal(cfg_dict.get("dca_min_pct", DCA_MIN_PCT))
    DCA_MAX_PCT = to_decimal(cfg_dict.get("dca_max_pct", DCA_MAX_PCT))
    DCA_FIXED_NOTIONAL_USD = to_decimal(cfg_dict.get("dca_fixed_notional_usd", DCA_FIXED_NOTIONAL_USD))
    DCA_TOTAL_CAP = int(cfg_dict.get("dca_total_cap", DCA_TOTAL_CAP))

    # 风控
    MARGIN_STOP_PCT = to_decimal(cfg_dict.get("margin_stop_pct", MARGIN_STOP_PCT))
    MARGIN_RESUME_PCT = to_decimal(cfg_dict.get("margin_resume_pct", MARGIN_RESUME_PCT))
    MARGIN_CHECK_SEC = int(cfg_dict.get("margin_check_sec", MARGIN_CHECK_SEC))

    # 新增：外部操作兼容/自恢复
    GRID_AUTOREPAIR = bool(cfg_dict.get("grid_autorepair", GRID_AUTOREPAIR))
    AUTOREPAIR_INTERVAL_SEC = int(cfg_dict.get("autorepair_interval_sec", AUTOREPAIR_INTERVAL_SEC))
    EXTERNAL_ADD_RECONCILE = bool(cfg_dict.get("external_add_reconcile", EXTERNAL_ADD_RECONCILE))
    RECONCILE_TOLERANCE_RATIO = to_decimal(cfg_dict.get("reconcile_tolerance_ratio", RECONCILE_TOLERANCE_RATIO))
    # 网格重装/扩展控制（热更新）
    GRID_ONE_SHOT = bool(cfg_dict.get("grid_one_shot", GRID_ONE_SHOT))
    # 离位重装 + 次数上限（热更新）
    REARM_ENABLED = bool(cfg_dict.get("rearm_enabled", REARM_ENABLED))
    REARM_HYSTERESIS_STEPS = to_decimal(cfg_dict.get("rearm_hysteresis_steps", REARM_HYSTERESIS_STEPS))
    REARM_USD_FLOOR = to_decimal(cfg_dict.get("rearm_usd_floor", REARM_USD_FLOOR))
    REARM_ALLOW_REPEATS = int(cfg_dict.get("rearm_allow_repeats", REARM_ALLOW_REPEATS))
    REARM_RESET_DISTANCE_LEVELS = int(cfg_dict.get("rearm_reset_distance_levels", REARM_RESET_DISTANCE_LEVELS))
    REARM_WINDOW_SEC = int(cfg_dict.get("rearm_window_sec", REARM_WINDOW_SEC))
    REARM_COOLDOWN_SEC = int(cfg_dict.get("rearm_cooldown_sec", REARM_COOLDOWN_SEC))
    REARM_COUNT_THRESHOLD_RATIO = to_decimal(cfg_dict.get("rearm_count_threshold_ratio", REARM_COUNT_THRESHOLD_RATIO))

    # 最大名义敞口
    MAX_NOTIONAL_USD_LONG  = to_decimal(cfg_dict.get("max_notional_usd_long", MAX_NOTIONAL_USD_LONG))
    MAX_NOTIONAL_USD_SHORT = to_decimal(cfg_dict.get("max_notional_usd_short", MAX_NOTIONAL_USD_SHORT))

    # 若步长或层数变化 → 撤旧并重建（仅 grid 非 None 时）
...
LOG_LEVEL = "INFO"
LOG_HEARTBEAT_SEC = 0
LOG_SUMMARY_EVERY_SEC = 60
LOG_CONSOLE_DAILY_SUMMARY = True
LOG_SUMMARY_TO_CSV = True
